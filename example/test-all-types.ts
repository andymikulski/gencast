...
import { CastToID, CastToUsername, CastToIsActive, CastToStatus, CastToPoint } from './Types.gen';

console.log('Testing Primitive and String Literal Type Casts\n');

// Test 1: Valid ID (number)
const validID: any = 123;
const id = CastToID(validID);
if (id === 123) {
...
const invalidID: any = "not-a-number";
const invalidIDResult = CastToID(invalidID);
...
const validUsername: any = "john_doe";
const username = CastToUsername(validUsername);
...
const invalidUsername: any = 123;
const invalidUsernameResult = CastToUsername(invalidUsername);
...
const validIsActiveTrue: any = true;
const isActiveTrue = CastToIsActive(validIsActiveTrue);
...
const validIsActiveFalse: any = false;
const isActiveFalse = CastToIsActive(validIsActiveFalse);
...
const invalidIsActive: any = "true";
const invalidIsActiveResult = CastToIsActive(invalidIsActive);
...
const validStatusActive: any = "active";
const statusActive = CastToStatus(validStatusActive);
...
const validStatusInactive: any = "inactive";
const statusInactive = CastToStatus(validStatusInactive);
...
const validStatusPending: any = "pending";
const statusPending = CastToStatus(validStatusPending);
...
const invalidStatusResult = CastToStatus(invalidStatus);
...
const invalidStatusNumber: any = 123;
const invalidStatusNumberResult = CastToStatus(invalidStatusNumber);
